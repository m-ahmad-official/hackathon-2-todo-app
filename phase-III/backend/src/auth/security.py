...
def authorize_user_for_conversation(
    ...
    if conversation.user_id != current_user_id:
        log_operation("CONVERSATION_AUTHORIZATION_DENIED", user_id=current_user_id, conversation_id=conversation.id)
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to access this conversation"
        )

    log_operation("CONVERSATION_AUTHORIZATION_GRANTED", user_id=current_user_id, conversation_id=conversation.id)
    return conversation


def log_security_event(event: str, user_id: str = "unknown", severity: str = "INFO", details: str = ""):
    """
    Log security-related events

    Args:
        event: The security event that occurred
        user_id: ID of the user involved (if any)
        severity: Severity level (INFO, WARNING, ERROR)
        details: Additional details about the event
    """
    message = f"SECURITY_EVENT: {event}"
    structured_logger.logger.log(
